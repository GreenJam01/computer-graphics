...
namespace Project1 {
	...
	enum projType { Ortho, Frustum, Perspective } pType; // тип трехмерной проекци
...
#pragma endregion
	...
	private: System::Void MyForm_KeyDown(System::Object^ sender, System::Windows::Forms::KeyEventArgs^ e) {
		float Wcx = MyForm::Wcx + Wx/ 2; // координаты центра
		float Wcy = MyForm::Wcy - Wy / 2; // текущего окна
		switch (e->KeyCode) {
		...
		case Keys::R: {				  //поворот наблюдателя относительно оси Oz
			vec3 u_new = mat3(rotate(0.1, vec3(0, 0, 1))) * vec3(0, 1, 0);
			T = lookAt(vec3(0, 0, 0), vec3(0, 0, -1), u_new) * T;
			break;
		}

		case Keys::Y: {				  //поворот наблюдателя относительно оси Oz
			vec3 u_new = mat3(rotate(-0.1, vec3(0, 0, 1))) * vec3(0, 1, 0);
			T = lookAt(vec3(0, 0, 0), vec3(0, 0, -1), u_new) * T;
			break;
		}
...
private: System::Void button1_Click(System::Object^ sender, System::EventArgs^ e) {
	if (openFileDialog->ShowDialog() == System::Windows::Forms::DialogResult::OK) {
		...
		if (in.is_open()) {
			...
			vector<mat4> transforms; // стек матриц преобразовани
			vector<path> figure; // список ломаных очередного рисунк
			...
			while (in) { // если очередная строка считана успешно
				...
				if ((str.find_first_not_of(" \t\r\n") != string::npos) && (str[0] != '#')) {
					// прочитанная строка не пуста и не комментарий									932231
					...
					if (cmd == "camera") { // положение камеры
						...
						fovy = fovy_work / 180.f * Math::PI; // перевод угла из градусов в радиантыv
					}
					...
					else if (cmd == "model") { // начало описания нового рисунка																										   чввцыч
						...
						M = translate(Tx, Ty, Tz) * M; // добавляем перенос к общему преобразовани
					}
						...
						transforms.push_back(M); // сохраняем матрицу в сте
					}
